...
![Basic ggplot syntax](images/ggplot%20syntax-01.png "GGplot syntax")
...
ggplot(data = Pine, mapping = aes(x = datetime, y = cfs))+
  geom_line()
# does not need to include mapping
# datetime is a POSIXct, which is the time data notation due to tidyverse 
```
...
ggplot(data = Pine, mapping = aes(x = datetime, y = cfs))+
  geom_point(pch = 8)
# hard coding numbers (like point numbers) happens outside of the aes and mapping, but happens in the geom 
```
...
# geom_point()
# this makes another column in the data and just filling it with blue, # says to control the color with blue as opposed to setting the color to # blue
...
xlab(element_blank())+ --\> which makes the label blank because the months are self explanatory

**Fourth: color by discharge**

color = cfs --\> won't show up because the blue is afterwards

```{r}
ggplot(data = Pine, aes(datetime ,cfs, color = cfs)) +
...
```{r}
ggplot(data = SNP, mapping = aes(x = datetime, y = cfs, color = StationID))+
  geom_line()
```
...
```{r}
ggplot(data = SNP, aes(datetime, cfs))+
  geom_line()+
  facet_wrap(facets = "StationID", nrow = 2)
# breaks them up by station ID becasue that is what the facets are
# keeps x and y axes the same
# if nrow = 1 then it will go to 2 columns, if you want a grid then you only need to do one of them because it is a limit of 2 rows and or 2 cols
...
# stationID by month
# default keeps the x and y axes range the same - set the scales are free to see all of the data more clearly by zooming in
  # can adjust jus the y and x axis with a similar command
...
```{r}
ggplot(data = SNP, mapping = aes(x = StationID, y = cfs)) +
  stat_boxplot() +
  scale_y_log10()

# data spans a few orders of magnitude, so we need to transfer to a log scale
# reading a boxplot - bold is median, upper is 75%, lower is 25%, points above are outliers
# can also use geom_boxplot()
```
...
```{r}
ggplot(data = RBI, aes(RBI, DRAIN_SQKM, size = T_AVG_SITE, bg = STATE)) +
...
# can stack different geometries, but they go in order so whatever you want on the top should be listed last
# method for stat_smooth, lm is for linear model and loess is for a moving average, and linetype 2 gives a dashed line
```
